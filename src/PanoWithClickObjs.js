...
var mat, panoSize, ratio_wh, infoWindow;
...
function init(s_material, bool_sphere = true) {
	...
	infoWindow.addEventListener("click", function() { isUserInteracting = false; });

	// Simple draggable implementation
	let dragHandle = infoWindow.querySelector(".handle");
	let isDragging = false, dragOffset = {x:0, y:0};
	dragHandle.addEventListener("mousedown", function(e) {
		isDragging = true;
		dragOffset.x = e.clientX - infoWindow.offsetLeft;
		dragOffset.y = e.clientY - infoWindow.offsetTop;
		infoWindow.style.height = "auto";
		document.body.style.userSelect = "none";
	});
	document.addEventListener("mousemove", function(e) {
		if (isDragging) {
			infoWindow.style.left = (e.clientX - dragOffset.x) + "px";
			infoWindow.style.top = (e.clientY - dragOffset.y) + "px";
		}
	});
	...
	if (bool_sphere){
		mesh = new THREE.Mesh(
			...
			new THREE.MeshBasicMaterial({ map: THREE.ImageUtils.loadTexture(mat) })
		);
	} else {
		mesh = new THREE.Mesh(
	...
	if (video) {
		video.addEventListener("mousedown", function(){ isUserInteracting = false; });
		video.addEventListener("mousemove", function(){ isUserInteracting = false; });
	}
...
function onWindowResize() {
	var container = document.getElementById("container");
	panoSize.w = container.offsetWidth;
	panoSize.h = container.offsetHeight;
	ratio_wh = panoSize.w / panoSize.h;
...
function onDocumentMouseDown(event) {
	...
	if (intersects.length > 0) {
		...
		document.getElementById("info").innerHTML = SELECTED.description;
		...
		if (video) {
			...
			if (SELECTED.vid) {
				...
				source.setAttribute("src", SELECTED.vid + ".mp4");
				video.load();
			} else {
